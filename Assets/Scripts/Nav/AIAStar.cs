...
public class AIAStar
{
    public static bool Generate(AINavNode startNode, AINavNode endNode, ref List<AINavNode> path)
    {
        ...
        startNode.Cost = 0;
        float hueristic = Vector3.Distance(startNode.transform.position, endNode.transform.position);
        ...
        while (!found && nodes.Count > 0)
        {
            ...
            foreach (var neighbor in node.neighbors)
            {
                ...
                if (cost > neighbor.Cost)
                {
                    ...
                    hueristic = Vector3.Distance(neighbor.transform.position, endNode.transform.position);

                    nodes.EnqueueWithoutDuplicates(neighbor, cost + hueristic);
                    break;
                }
            }
        }
